...
def vns_algorithm(routes, times, capacity, max_iterations=100):
    """
    Algoritmo VNS mejorado para explorar vecindarios de manera más agresiva.
    """
    best_routes = [route.copy() for route in routes]
    best_distance = calculate_total_distance(best_routes, times)
    iteration = 0
    ...
    for idx, route in enumerate(best_routes):
        route_improved, route_distance, route_changed = two_opt_within_route_single(route, times, capacity)
        if route_changed:
            best_routes[idx] = route_improved
...
def vrptw_improve_solutions(excel_filename, directory_path, output_filename):
    ...
    for i in range(1, 19):
        ...
        # Aplicar VNS estricto para mejorar la solución
        file_start_time = time.time()
        improved_routes = vns_algorithm(routes, times, Q)
        improved_distance = calculate_total_distance(improved_routes, times)
        improvement_time = (time.time() - file_start_time) * 1000  # Tiempo en milisegundos
...
if __name__ == '__main__':
    ...
    output_filename = "VRPTW_JuanFernando__aco_VNS.xlsx"
